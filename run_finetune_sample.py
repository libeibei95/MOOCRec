"""
Script for fine-tuning the pre-trained model and evaluating using negative sampling
...
from datasets import FineTrainDataset
...
def main():
    ...
    args.sample_file = os.path.join(args.data_dir, args.data_name + '_sample.csv')
    ...
    train_dataset = FineTrainDataset(args, user_seq, data_type='train')
    train_sampler = RandomSampler(train_dataset)
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.batch_size)

    eval_dataset = FineTrainDataset(args, user_seq, test_neg_items=sample_seq, data_type='valid')
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size)

    test_dataset = FineTrainDataset(args, user_seq, test_neg_items=sample_seq, data_type='test')
    ...
    if args.do_eval:
        ...
        pretrained_path = os.path.join(args.output_dir, f'Pretrain-{args.data_name}-epochs-{args.ckp}.pt')
        try:
